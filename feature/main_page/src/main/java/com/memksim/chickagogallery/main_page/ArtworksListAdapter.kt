...
import coil.load
import com.memksim.chickagogallery.main_page.databinding.ArtworkItemLayoutBinding
import com.memksim.chickagogallery.view_extenstion.setBookmarkIconDrawable

internal class ArtworksDiffCallback(
    private val oldList: List<MainPageUiState.MainPageItemUiState>,
    private val newList: List<MainPageUiState.MainPageItemUiState>
) : DiffUtil.Callback() {
...
internal class ArtworksAdapter(
    private val onCLick: (artwork: MainPageUiState.MainPageItemUiState) -> Unit
) : RecyclerView.Adapter<ArtworksAdapter.ArtworksViewHolder>() {

    var items: MutableList<MainPageUiState.MainPageItemUiState> = mutableListOf()
        set(value) {
